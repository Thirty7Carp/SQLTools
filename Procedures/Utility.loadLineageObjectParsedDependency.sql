...
BEGIN
    ...
    DECLARE @DDLKeywords TABLE (Keyword NVARCHAR(50), OperationType NVARCHAR(50));
    ...
    BEGIN
        ...
        DECLARE @DDLOpType NVARCHAR(50);

        DECLARE ddl_cur CURSOR FOR
            SELECT Keyword, OperationType FROM @DDLKeywords;

        OPEN ddl_cur;
        FETCH NEXT FROM ddl_cur INTO @DDLKeyword, @DDLOpType;
        ...
        BEGIN
            ...
            WHILE @SearchPos <= @DefLen
            BEGIN
                DECLARE @DDLPos INT = CHARINDEX(@DDLKeyword, @Definition, @SearchPos);

                IF @DDLPos = 0
                    BREAK;

                SET @SearchPos = @DDLPos + LEN(@DDLKeyword);
