...
- A database is offline and you need to know what is impacted.
- A table fails to load due to an object definition changing.
- A stored procedure fails due to a a script update.
- A field value changes, and you need to know what might be affected.
- etc.

This tracking will not capture 100% of your system dependencies, but it will quickly establish the relationships affected in most tables, views, and sprocs.

Things it won't work with:
- MS DB prior to 2016 - though I can work on it if you have a demand.
- Dynamic SQL driven by variables.
...
- Create Utility.LNG_DatabaseExclusions  - A place to list databases you want exclued from lineage tracking.
...
- Create Utility.LNG_ObjectExpressionDependency - Stored the output from sys.sql_expression_dependencies that can be used for lineage. Used purely for checking effectiveness of LNG objects.
...
- Create Utility.LNG_loadObjectList - Loads LNG_ObjectList. - You need to enter the @UtilitySchemaDatabase variable with your database name.
...
- Create Utility.LNG_updateObjectExclusionsAdd - Add Objects to LineageDatabaseExclusions. Can use Wildcard on the Objectname to match patterns, or exclude an entire schema.
